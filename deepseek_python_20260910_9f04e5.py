...
def load_model():
    if not os.path.exists(MODEL_PATH):
        with st.spinner("📥 Model download ho raha hai (pehli baar 2-3 minute lagenge)..."):
...
with st.sidebar:
    ...
    st.info("""
    **Yeh platform:**
    - AI se disease detect karta hai
    - Data research ke liye save karta hai
    - Students papers mein use kar sakte hain
    """)
    ...
    if os.path.exists('dataset.json'):
        ...
        if st.button("📥 Download Dataset (JSON)"):
            st.download_button(
                "Click to Download",
                data=json.dumps(data, indent=2),
                file_name="crop_dataset.json",
